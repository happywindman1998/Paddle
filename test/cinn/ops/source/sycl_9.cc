...
void fn_max_0_kernel(sycl::queue &Q, sycl::range<3> dimGrid, sycl::range<3> dimBlock, void** void_args) {
  ...
  Q.submit([&](sycl::handler &h) {
    h.parallel_for<class space9_fn_max_0_kernel>(sycl::nd_range<3>(dimGrid * dimBlock, dimBlock), [=](sycl::nd_item<3> item) [[intel::kernel_args_restrict]][[intel::max_work_group_size(1, 1, 1)]]
    {
      var_1[0] = sycl::max(x[0], y[0]);
    });
  });
}
