...
function(cinn_sycl_library TARGET_NAME)
  if(WITH_GPU)
    set(options STATIC static SHARED shared)
    set(oneValueArgs "")
    set(multiValueArgs SRCS DEPS)
    cmake_parse_arguments(cinn_sycl_library "${options}" "${oneValueArgs}"
                          "${multiValueArgs}" ${ARGN})
    if(cinn_sycl_library_SRCS)
      if(cinn_sycl_library_SHARED OR cinn_sycl_library_shared) # build *.so
        add_library(${TARGET_NAME} SHARED ${cinn_sycl_library_SRCS})
      else()
        add_library(${TARGET_NAME} STATIC ${cinn_sycl_library_SRCS})
      endif()
      if(cinn_sycl_library_DEPS)
        add_dependencies(${TARGET_NAME} ${cinn_sycl_library_DEPS})
        target_link_libraries(${TARGET_NAME} ${cinn_sycl_library_DEPS})
      endif()
      # cpplint code style
      foreach(source_file ${cinn_sycl_library_SRCS})
        string(REGEX REPLACE "\\.[^.]*$" "" source ${source_file})
        if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${source}.h)
          list(APPEND cinn_sycl_library_HEADERS
               ${CMAKE_CURRENT_SOURCE_DIR}/${source}.h)
        endif()
      endforeach()
    else()
      if(cinn_sycl_library_DEPS)
        cinn_merge_static_libs(${TARGET_NAME} ${cinn_sycl_library_DEPS})
      else()
        message(FATAL
                "Please specify source file or library in cinn_sycl_library.")
      endif()
    endif()
    target_link_libraries(${TARGET_NAME} Threads::Threads)
  endif()
