...
std::array<int, 3> Target::get_max_grid_dims() const {
  return std::get<std::array<int, 3>>(
      BackendAPI::get_backend(language)->get_device_property(
          BackendAPI::DeviceProperty::MaxGridDims));
}
std::array<int, 3> Target::get_max_block_dims() const {
  return std::get<std::array<int, 3>>(
      BackendAPI::get_backend(language)->get_device_property(
          BackendAPI::DeviceProperty::MaxBlockDims));
}
