...
struct IndexVec {
  ...
  inline static self_type Ramp(int32_t base, int32_t stride = 1, int32_t denom = 1) {
    self_type res;
    res.base = base;
    res.stride = stride;
    res.denom = denom;
...
struct DataVec {
  ...
  inline DataVec(const DataVec<T, Num> &other) {
    data_ = other.data_;
  }
  ...
  inline static self_type Load(const value_type* base, const IndexVec<Num>& offset) {
    self_type res;
    sycl::ext::mlu::memcpy_gdram2nram(res.data_, base, Num, offset.base, offset.denom);
    return res;
  }
  ...
  inline self_type operator+(const self_type& other) const {
    self_type res;
    sycl::ext::mlu::vector_add(res.data_, data_, other.data_, Num);
    return res;
  }

  inline self_type operator+(value_type val) const {
    self_type res;
    sycl::ext::mlu::vector_add(res.data_, data_, val, Num);
    return res;
  }
  ...
  inline self_type operator-(const self_type& other) const {
    self_type res;
    sycl::ext::mlu::vector_sub(res.data_, data_, other.data_, Num);
    return res;
  }

  inline self_type operator-(value_type val) const {
    self_type res;
    sycl::ext::mlu::vector_sub(res.data_, data_, val, Num);
    return res;
  }
  ...
  inline self_type operator*(const self_type& other) const {
    self_type res;
    sycl::ext::mlu::vector_mul(res.data_, data_, other.data_, Num);
    return res;
  }

  inline self_type operator*(value_type val) const {
    self_type res;
    sycl::ext::mlu::vector_mul(res.data_, data_, val, Num);
    return res;
  }
  ...
  inline self_type operator/(const self_type& other) const {
    self_type res;
    sycl::ext::mlu::vector_div(res.data_, data_, other.data_, Num);
    return res;
  }

  inline self_type operator/(value_type val) const {
    self_type res;
    sycl::ext::mlu::vector_div(res.data_, data_, val, Num);
    return res;
  }
  ...
  inline self_type operator%(const self_type& other) const {
    self_type res;
    sycl::ext::mlu::vector_mod(res.data_, data_, other.data_, Num);
    return res;
  }

  inline self_type operator%(value_type val) const {
    self_type res = Broadcast(val);
    sycl::ext::mlu::vector_mod(res.data_, data_, res.data_, Num);
    return res;
  }

  inline friend self_type operator%(value_type val, const self_type& vec) {
    self_type res = Broadcast(val);
    sycl::ext::mlu::vector_mod(res.data_, res.data_, vec.data_, Num);
    return res;
  }
...
template <typename T, size_t Num>
inline DataVec<T, Num> cinn_sycl_select(const DataVec<T, Num> &condition, const DataVec<T, Num> &true_val,
...
#define DEF_FN_FP32_VEC_BINARY(func)                                               \
template <size_t Num>                                                              \
inline DataVec<float, Num> cinn_sycl_##func##_fp32(const DataVec<float, Num> &x,   \
...
inline DataVec<float, Num> cinn_sycl_##func##_fp32(const DataVec<float, Num> &x,   \
                                                   float y) {                      \
  DataVec<float, Num> res = DataVec<float, Num>::Broadcast(y);                     \
  sycl::ext::mlu::vector_##func(res.data_, x.data_, res.data_, Num);               \
  return res;                                                                      \
}                                                                                  \
template <size_t Num>                                                              \
inline DataVec<float, Num> cinn_sycl_##func##_fp32(float x,                        \
                                                   const DataVec<float, Num> &y) { \
  DataVec<float, Num> res = DataVec<float, Num>::Broadcast(x);                     \
  sycl::ext::mlu::vector_##func(res.data_, res.data_, y.data_, Num);               \
  return res;                                                                      \
}

#define DEF_FN_FP32_VEC_SCALAR_BINARY(func)                                        \
template <size_t Num>                                                              \
inline DataVec<float, Num> cinn_sycl_##func##_fp32(const DataVec<float, Num> &x,   \
                                                   const DataVec<float, Num> &y) { \
  DataVec<float, Num> res;                                                         \
...
DEF_FN_FP32_VEC_UNARY(sin)
DEF_FN_FP32_VEC_UNARY(sinh)
DEF_FN_FP32_VEC_UNARY(sqrt)
DEF_FN_FP32_VEC_UNARY(tan)
...
inline float cinn_sycl_max(float a, float b) { return sycl::max(a, b); }
inline int cinn_sycl_max(int a, int b) { return sycl::max(a, b); }
inline float cinn_sycl_min(float a, float b) { return sycl::min(a, b); }
inline int cinn_sycl_min(int a, int b) { return sycl::min(a, b); }

template <size_t Num>
inline DataVec<float, Num> cinn_sycl_max(const DataVec<float, Num> &x, const DataVec<float, Num> &y) {
  DataVec<float, Num> res;
  sycl::ext::mlu::vector_max(res.data_, x.data_, y.data_, Num);
  return res;
}

template <size_t Num>
inline DataVec<float, Num> cinn_sycl_max(const DataVec<float, Num> &x, float y) {
  DataVec<float, Num> res;
  sycl::ext::mlu::vector_max(res.data_, x.data_, y, Num);
  return res;
}

template <size_t Num>
inline DataVec<float, Num> cinn_sycl_max(float x, const DataVec<float, Num> &y) {
  DataVec<float, Num> res;
  sycl::ext::mlu::vector_max(res.data_, x, y.data_, Num);
  return res;
}

template <size_t Num>
inline DataVec<float, Num> cinn_sycl_min(const DataVec<float, Num> &x, const DataVec<float, Num> &y) {
  DataVec<float, Num> res;
  sycl::ext::mlu::vector_min(res.data_, x.data_, y.data_, Num);
  return res;
}

template <size_t Num>
inline DataVec<float, Num> cinn_sycl_min(const DataVec<float, Num> &x, float y) {
  DataVec<float, Num> res;
  sycl::ext::mlu::vector_min(res.data_, x.data_, y, Num);
  return res;
}

template <size_t Num>
inline DataVec<float, Num> cinn_sycl_min(float x, const DataVec<float, Num> &y) {
  DataVec<float, Num> res;
  sycl::ext::mlu::vector_min(res.data_, x, y.data_, Num);
  return res;
}
