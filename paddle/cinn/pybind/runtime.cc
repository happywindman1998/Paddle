...
cinn_buffer_t *CreateBufferFromNumpy(
  ...
  if (target == common::DefaultHostTarget()) {
    ...
    buffer->memory = static_cast<uint8_t *>(BackendAPI::get_backend(target)->malloc(data.nbytes()));
    BackendAPI::get_backend(target)->memcpy(buffer->memory, data.data(), data.nbytes(), BackendAPI::MemcpyType::HostToDevice);
    return buffer;
  } else {
