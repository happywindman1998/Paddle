...
CINN_REGISTER_HELPER(op_external_api) {
...
#if defined(CINN_WITH_SYCL) && defined(CINN_WITH_ONEDNN)
  ...
  CINN_OP_REGISTER_EXTERNAL_API(mul, sycl_dev)
      .set_api_name("cinn_call_onednn_matmul");
  CINN_OP_REGISTER_EXTERNAL_API(conv2d, sycl_dev)
      .set_trans_func([](const ::cinn::hlir::framework::Node* node) {
        CHECK(node->attrs.attr_store.count("conv_type"));
        std::string conv_type =
            absl::get<std::string>(node->attrs.attr_store.at("conv_type"));
  ...
  CINN_OP_REGISTER_EXTERNAL_API(depthwise_conv2d, sycl_dev)
      .set_trans_func([](const ::cinn::hlir::framework::Node* node) {
        std::string conv_type =
            node->attrs.attr_store.count("conv_type")
                ? absl::get<std::string>(node->attrs.attr_store.at("conv_type"))
  ...
  CINN_OP_REGISTER_EXTERNAL_API(pool2d, sycl_dev)
      .set_api_name("cinn_call_onednn_pool2d_forward");
  CINN_OP_REGISTER_EXTERNAL_API(pool2d_grad, sycl_dev)
